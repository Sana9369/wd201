...
describe("Tests for functions in todo.js", function () { beforeAll(async () => { await store.sequelize.sync({ force: true })});
test("Todo.overdue should return all tasks (including completed ones) that are past their due date", async () => {
const todo = await store.Todo.addTask({ title: "This is a sample item", dueDate: getJSDate(-2), completed: false });
const items = await store.Todo.overdue();
expect(items.length).toBe(1); });
test("Todo.dueToday should return all tasks that are due today (including completed ones)", async () => {
const dueTodayItems = await store.Todo.dueToday();
const todo = await store.Todo.addTask({ title: "This is a sample item", dueDate: getJSDate(0), completed: false });
const items = await store.Todo.dueToday();
...
const dueLaterItems = await store.Todo.dueLater();
const todo = await store.Todo.addTask({ title: "This is a sample item", dueDate: getJSDate(2), completed: false });
const items = await store.Todo.dueLater();
...
const overdueItems = await store.Todo.overdue()
const aTodo = overdueItems[0];
expect(aTodo.completed).toBe(false);
await store.Todo.markAsComplete(aTodo.id);
await aTodo.reload();
